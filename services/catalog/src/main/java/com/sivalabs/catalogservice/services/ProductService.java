...
public class ProductService {
    ...
    @Cacheable(value = "catalog-products", key = "'all'")
    public List<Product> findAllProducts() {
        List<Product> products = productRepository.findAll();
        final Map<String, Integer> inventoryLevels = getInventoryLevelsWithFeignClient();
        products.forEach(p -> {
            Integer quantity = inventoryLevels.get(p.getCode());
            p.setInStock(quantity != null && quantity > 0);
        });
        return products;
    }
    ...
    @Cacheable(value = "catalog-products", key = "#code")
