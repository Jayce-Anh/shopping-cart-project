...
resource "aws_iam_policy" "alb_controller" {
  name        = format("%s-alb-controller-policy", var.eks_name)
  ...
  tags = merge(var.tags, {
    Name = "${var.project.env}-${var.project.name}-alb-controller-policy"
  })
}

resource "aws_iam_role" "alb_controller" {
  name = format("%s-alb-controller-role", var.eks_name)
  ...
  tags = merge(var.tags, {
    Name = "${var.project.env}-${var.project.name}-alb-controller-role"
  })
}
